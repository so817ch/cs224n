...
class PartialParse(object):
    def __init__(self, sentence: list[str]):
        ...
        self.buffer: list[str] = [item for item in self.sentence]
...
def minibatch_parse(sentences:list[list[str]], model, batch_size):
    ...
    def is_finished(parse:PartialParse):
        return len(parse.stack)==1 and len(parse.buffer)==0
    ...
    while len(unfinished_parses):
        ...
        unfinished_parses[:batch_size]=[parse for parse in unfinished_parses[:batch_size] if not is_finished(parse)]
