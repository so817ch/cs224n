...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.hidden_to_logits_weight = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(self.hidden_size, n_classes)))
        self.hidden_to_logits_bias = nn.Parameter(
            nn.init.uniform_(torch.empty(1, self.n_classes)))
       ###
    ...
    def forward(self, w):
        ...
        h = F.relu(self.embedding_lookup(
            w)@self.embed_to_hidden_weight+self.embed_to_hidden_bias)
        h = self.dropout(h)
