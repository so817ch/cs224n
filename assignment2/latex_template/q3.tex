...
\begin{parts}
    ...
    \ifans{
        A sentence containing $n$ words will be parsed in $2n$ steps. First there will be $n$ steps of \texttt{*-ARC} operation to build a dependency tree (including the ROOT and each word of the sentence) of $n+1$ words in the stack, since there are $n$ edges in a $n+1$-node-tree. Second, we need another $n$ steps of \texttt{SHIFT} option to move all the $n$ words of the sentence from the buffer to the stack.
    }
    ...
    \ifans{
        ...
        Part-of-speech (POS) tags offer valuable grammatical information for each word, aiding the dependency parser in grasping the syntactic structure of the sentence. By clarifying the grammatical role of words (e.g., verb, noun), POS tags help disambiguate words with multiple meanings in different contexts. Additionally, they enhance contextual understanding by enabling the parser to identify subjects, objects, modifers, and compound nouns, there by improving overall parsing accuracy.
    }
